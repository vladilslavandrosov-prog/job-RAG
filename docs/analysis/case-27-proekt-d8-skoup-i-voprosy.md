...
- **К заказчику — 15 вопросов и 6 наших предложений** с рабочей позицией по умолчанию (проще поправить готовое предложение, чем отвечать с нуля) — §5–6.
...
- **Пилот формально не прошёл все пороги приёмки** (Сценарий 6: 67,4%/70%, Сценарий 4: 87,5%/95%, `case-21`) — предлагаем не ждать дозакрытия, а перенести оба пункта в список долга бизнес-кейса П1 (предложение 6, §6, вопрос 15).
...
Оба вопроса «что входит в волну» (§5, вопросы 1 и 3) сейчас сформулированы как открытые — они спрашивают **что**, но не **сколько**. Без числовых параметров ниже нельзя оценить трудоёмкость волны даже приблизительно (а Кейс 2 методики и так не даёт оценок трудоёмкости из-за отсутствия прецедента — см. `docs/metodika-vnedreniya.md`, §4.3; закрыть эти 4 параметра — необходимое условие, чтобы вообще начать оценку, когда проект перейдёт в фазу П2).
...
|---|---|---|---|
| Состав данных | 4 таблицы: `crm.crm.crm.clients`, `crm.crm.crm.managers`, `trade.trade.trading.mandates_targets`, `trade.trade.trading.portfolios` (у последней — известный пробел: значение портфеля не находится, хотя источник подключён, см. `case-01-d8-014-brief.md`) | Новые витрины/таблицы: домен, ключевые поля, владелец на стороне Д8, объём (строк / размер) | Вопрос 3 |
| Количество витрин | 4 (пилот) | Точное число витрин сверх пилотных 4 — сейчас вопрос 3 спрашивает список, но не просит явно назвать количество | Вопрос 3 — дополнить формулировку |
| API в шине | 1 интеграция: CRM Bitrix24, REST API/Webhook | Список новых API: система-источник, формат (REST/SOAP/др.), ответственный на стороне Д8 за контракт | Вопрос 4 |
| Количество пользователей | до 20 (пилот, все роли вместе) | Сколько пользователей получат доступ именно к расширенным данным волны 2 — не общий штат, а конкретный контур волны | Новый пункт — добавить к вопросу 6 |
| Количество ролей | 2 из «до 5» по пилотному ТЗ | Волна 2 работает в рамках тех же 2 ролей, что пилот, или подключает новые | Вопрос 6 |
...
| Количество пользователей | до 20 (пилот) | Тот же контур пользователей, что в пилоте, или волна 1 открывает доступ новым | Новый пункт — добавить к вопросу 6 |
| Количество ролей | 2 (пилот) | Какие роли получают доступ к волне 1 | Вопрос 6 |

**Как использовать:** это рабочий шаблон сбора данных, не готовый вопрос заказчику — заполняется по итогам созвона по волне 1 (предложение 1, §6) и по ответам на вопросы 3–6. Тот же шаблон (4 параметра) переиспользуется для волны 3+, когда её содержание определится.
...
3. Какие конкретно витрины данных нужно добавить в ESB сверх текущего среза CRM (Bitrix24)? Нужен список с владельцами на стороне Д8 — и явное число: сколько витрин добавляется сверх пилотных 4 (`crm.crm.crm.clients`, `crm.crm.crm.managers`, `trade.trade.trading.mandates_targets`, `trade.trade.trading.portfolios`).
...
8. Права доступа сотрудников сейчас фиксированы по статичному списку политик. Для проекта это должно стать динамическим через API — но нужна точная схема процесса: права хранятся и администрируются внутри самой платформы (и меняются там же), или платформа только получает их из внешней системы-источника через API и применяет как есть? Если второе — какая система источник истины (HR-система, Bitrix24, отдельный IAM) и как часто права обновляются? *(= пункт реестра «Обновление информации о правах доступа...», Уточнить у заказчика)*
...
11. Кастомизация выбора источников — нужно уточнить, что именно имеется в виду: (а) подключение/отключение целых каталогов данных внутри домена компании, или (б) выбор персоны/профиля пользователя с отдельным набором прав доступа к источникам? Это разные по объёму реализации задачи.
12. Персональные RAG-коллекции пользователей — нужно уточнить механику: имеется в виду фильтр по документам внутри уже существующей коллекции (сузить область поиска), или отдельная новая коллекция, которую пользователь создаёт и наполняет сам?
13. Упрощение заведения пользователей — нужен не общий запрос, а конкретный кейс: как должен выглядеть процесс пошагово (кто инициирует, какие поля заполняются, кто подтверждает), чтобы платформа могла спроектировать решение по существу.
