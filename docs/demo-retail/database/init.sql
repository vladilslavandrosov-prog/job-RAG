...
-- =========================================================================
-- 10. Категории чувствительности и каталог полей — опора для настройки
--     масок AI Guard / Графа доступа. Раньше значения 'pii' / 'financial' /
--     'contract_terms' были описаны только в SQL-комментарии (невидимом
--     любому, кто просто читает таблицу) — теперь это отдельная таблица
--     с описанием и рекомендацией по маскированию, на неё ссылается FK.
-- =========================================================================

CREATE TABLE sensitivity_levels (
    sensitivity_code  TEXT PRIMARY KEY,
    description       TEXT NOT NULL,
    masking_guidance  TEXT NOT NULL
);

CREATE TABLE data_sensitivity_catalog (
    table_name      TEXT NOT NULL,
    column_name     TEXT NOT NULL,
    sensitivity     TEXT NOT NULL REFERENCES sensitivity_levels(sensitivity_code),
    note            TEXT,
...
INSERT INTO sensitivity_levels (sensitivity_code, description, masking_guidance) VALUES
    ('pii',
     'Персональные данные — прямо идентифицируют конкретного человека: ФИО, телефон, e-mail, номер карты лояльности, юридический адрес.',
     'Маскировать для любой роли, которой не нужно знать личность конкретного человека для выполнения задачи (например, специалист поддержки видит историю покупок, но не полный номер карты). Кандидат на контекстный BERT-фильтр — распознавание по типу сущности (ФИО/телефон/e-mail), а не по конкретному списку значений.'),
    ('financial',
     'Финансовые и регистрационные данные компании — банковские реквизиты, ИНН/КПП/ОГРН, закупочные цены, себестоимость, маржа.',
     'Маскировать для ролей без доступа к финансовым показателям (например, «директор магазина» видит выручку своего магазина, но не закупочную стоимость и маржу — см. Сценарий 2). Для банковских реквизитов поставщика — видно только ролям, ответственным за оплату.'),
    ('contract_terms',
     'Содержательные условия договора, а не отдельный персональный/финансовый атрибут — например, весь раздел «Гарантийные обязательства».',
     'Маскировать целиком по явному, настроенному правилу (не по regex общего назначения, который совпадает по формату числа/даты) — раздел скрывается независимо от конкретной формулировки. См. демо-Сценарий 9: тест «жёсткое / мягкое маскирование» именно на этом типе данных, потому что ошибочно скрытое условие меняет правовой смысл ответа, а не просто прячет цифру.');
